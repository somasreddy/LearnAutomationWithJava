...
public class GenerateReportFromRQM {
	...
	public static void main(String[] args) throws IOException, InterruptedException, InvalidFormatException {
		...
		for (int i = 1; i <= LastRow; i++) {
			...
			System.out.println(gRFR.searchAndSelectTestPlan(driver, gRFR.GetTPNameFromExcel(wb,sheetName,i,0)+releaseNum,wait));
	...
	public String GetTPNameFromExcel(Workbook wb, String Sheet, int Row, int Col) { return wb.getSheet(sheetName).getRow(Row).getCell(Col).toString(); }
	...
	public void clickExport(WebDriver driver, WebDriverWait wait) throws InterruptedException {
		...
		List<WebElement> lstExport=(List<WebElement>)wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.xpath(exportIcon)));
		...
		for(WebElement export:lstExport) {
			...
			try {
				...
				try {
					exportAllPages=(WebElement)wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//td[.='Export All Pages']")));	
				}
				catch(Exception e) {			}
				...
				if (exportAllPages != null) {
					...
					WebElement exportToCSV=(WebElement)wait.until(ExpectedConditions.elementToBeClickable(By.xpath(exportCSVPath)));
	...
	public String searchAndSelectTestPlan(WebDriver driver, String testPlanName, WebDriverWait wait) throws InterruptedException {
		...
		try {
			...
			driver.findElement(By.xpath("//img[@alt='Filter']/../../input[@title='Type filter text or ID']/../button[@title='Clear Filter Text']")).click();
		}
		...
		if(filterSearchTB!=null) {
			...
			filterSearchTB=driver.findElement(By.xpath("//img[@alt='Filter']/../../input[@title='Type filter text or ID']"));
		...
		String testPlanSearchXpath="//div[contains(text(),'"+testPlanName+"') and not(contains(text(),'"+testPlanName+"-')) and not(contains(text(),'"+testPlanName+".')) and not(contains(text(),'"+testPlanName+"_'))]";

		try {

			testPlanSearchResult= (WebElement)wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(testPlanSearchXpath)));
		...
		if(testPlanSearchResult!=null) {
			...
			WebElement tpSearchResult=(WebElement)wait.until(ExpectedConditions.elementToBeClickable(By.xpath(testPlanSearchXpath+"/../../../../preceding-sibling::td/span/div[@class='dijit dijitReset dijitInline dijitCheckBox']/input[@role='checkbox']")));
	...
	public void loginToRQM(WebDriver driver, String username, String password,WebDriverWait wait) throws IOException, InterruptedException {
		...
		WebElement userName=(WebElement)wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("input[id*='userId'][name*='username']")));
		userName.click();
		userName.click();
		...
		WebElement rememberMeChkBox = driver.findElement(By.cssSelector("input[name ='rememberUserId'][type='checkbox']"));
	...
	public void logOutRQM(WebDriver driver ,WebDriverWait wait) throws InterruptedException {
		...
		WebElement profileBtn=wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//span[@class='user-name']/../img")));
