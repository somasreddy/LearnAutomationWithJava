...
public class ModifyJenkinsJob {
	...
	try {
		...
		chromeDriver.findElement(By.xpath("//input[@id='j_username']"))
			.sendKeys(new CharSequence[] { userName });
		System.out.println("Please Enter the Password : ");
		String password = in.nextLine();
		chromeDriver.findElement(By.xpath("//input[@placeholder='Password']"))
	...
	Thread.sleep(1500L);
	List<WebElement> lst = chromeDriver.findElements(By.xpath("//tr[contains(@id,'job_EP_')]/td[3]/a"));
	for (int i = 0; i < lst.size(); i++) {
	    String mainWindow = chromeDriver.getWindowHandle();
	    // String job =
	    // ((WebElement)lst.get(i)).findElement(By.xpath("..")).getAttribute("id");
		...
		if (!mainWindow.equals(childWindow)) {
		    ...
		    Thread.sleep(1500L);
		    WebElement emailID = chromeDriver.findElement(By.name("_.recipients"));
		    emailID.clear();
		    emailID.sendKeys("Somasekhar.v@cerner.com");

		    //
		    // boolean textFinder = false;
		    // try {
		    // textFinder = chromeDriver.findElement(By.xpath("//td/div/b[.='Text
		    // Finder']")).isDisplayed();
		    // }
		    // catch (NoSuchElementException noSuchElementException) {}
		    // if (textFinder) {
		    // WebElement saveButton =
		    // chromeDriver.findElement(By.xpath("//span/button[.='Save']"));
		    // saveButton.click();
		    // chromeDriver.manage().timeouts().implicitlyWait(10L, TimeUnit.SECONDS);
		    // chromeDriver.close();
		    // continue;
		    // }
		    // WebElement buttonPBA =
		    // chromeDriver.findElement(By.xpath("//span/button[.='Add post-build
		    // action']"));
		    // buttonPBA.click();
		    // Thread.sleep(1500L);
		    ...
		    // chromeDriver.findElement(By.xpath("//td/input[@name='_.regexp']")).sendKeys(new
		    // CharSequence[] { "RQMGO" });
