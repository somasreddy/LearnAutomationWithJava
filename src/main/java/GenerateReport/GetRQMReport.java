...
public class GetRQMReport {
	...
	public static void main(String[] args) throws InterruptedException, IOException, InvalidFormatException {
		...
		WebDriverWait wait = new WebDriverWait((WebDriver)chromeDriver, 60000L);
		...
		for (int i = 1; i <= LastRow; i++)
			System.out.println(gRR.searchAndSelectTestPlan((WebDriver)chromeDriver, String.valueOf(gRR.GetTPNameFromExcel(wb, sheetName, i, 0)) + releaseNum)); 
		List<WebElement> lstExport = chromeDriver.findElements(By.xpath("//img[@alt='Download as spreadsheet (.csv)']"));
		for (WebElement export : lstExport) {
			try {
				export.click();
			} catch (Exception exception) {}
		} 
		WebElement exportToCSV = (WebElement)wait.until((Function)ExpectedConditions.elementToBeClickable(By.xpath("//td/div[@title='1 minute ago']/../../td/a[contains(@href,'IPrintService')]")));
		exportToCSV.click();
		Thread.sleep(5000L);
	...
	public void navigateToFilterView(WebDriver driver) {
		...
		WebElement testPoint = (WebElement)wait.until((Function)ExpectedConditions.elementToBeClickable(By.xpath("//div[contains(.,'Total:')]/preceding-sibling::div[contains(.,'Progress')]/following-sibling::div/../..")));
		testPoint.click();
		WebElement selectedTP = (WebElement)wait.until((Function)ExpectedConditions.elementToBeClickable(By.xpath("//div[contains(text(),'In any test plan')]/../../following-sibling::tr/td/span/div[@class='dijit dijitReset dijitInline dijitCheckBox dijitCheckBoxChecked dijitChecked']")));
		selectedTP.click();
	}
	...
	public String searchAndSelectTestPlan(WebDriver driver, String testPlanName) {
		...
		if (testPlanSearchResult != null) {
			WebElement tpSearchResult = (WebElement)wait.until((Function)ExpectedConditions.elementToBeClickable(By.xpath("//div[contains(text(),'" + testPlanName + "')]/../../../../preceding-sibling::td/span/div[@class='dijit dijitReset dijitInline dijitCheckBox']")));
	...
	public void loginToRQM(WebDriver driver, String username, String password) throws IOException, InterruptedException {
		...
		WebElement usernameTxtBx = (WebElement)wait.until((Function)ExpectedConditions.elementToBeClickable(By.cssSelector("input[id*='userId'][name*='username']")));
		usernameTxtBx.sendKeys(new CharSequence[] { username });
		WebElement pswrdTxtBx = (WebElement)wait.until((Function)ExpectedConditions.elementToBeClickable(By.xpath("//input[@name='j_password']")));
	...
	public void LogOutRQM(WebDriver driver) throws IOException, InterruptedException {
		...
		builder.sendKeys(new CharSequence[] { (CharSequence)Keys.PAGE_UP }).build().perform();
		Thread.sleep(5000L);
		WebElement logoutbtn = (WebElement)wait.until((Function)ExpectedConditions.elementToBeClickable(By.cssSelector("span[class='caret jazz-ui-MenuPopup-caret']")));
		logoutbtn.click();
		Thread.sleep(15000L);
		WebElement logout = (WebElement)wait.until((Function)ExpectedConditions.elementToBeClickable(By.xpath("//span[@data-dojo-attach-point='containerNode'][contains(@id,'text')][contains(.,'Log Out')]")));
		logout.click();
